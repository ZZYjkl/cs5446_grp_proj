...
def trainNetwork(s, readout, sess):
    ...
    if checkpoint and checkpoint.model_checkpoint_path and not not os.listdir("saved_networks"):
    ...
    while game_state.getLifes() <= game.TOTAL_LIFES:
        ...
        s_t = s_t1
        t += 1
    return game_state.getSteps(), game_state.getRewards(), game_state.getIntReward()
        
def playGame():
    sess = tf.InteractiveSession()
    s, readout = createNetwork()
    steps, rewards, int_reward = trainNetwork(s, readout, sess)
    lifes = np.arange(1, game.TOTAL_LIFES+1)

    print(int_reward)

    plt.plot(lifes, steps)
    plt.xlabel("Lifes")
    plt.ylabel("Steps Survived")
    plt.savefig("step_survived.png")
    plt.close()

    plt.plot(lifes, rewards)
    plt.xlabel("Lifes")
    plt.ylabel("Rewards")
