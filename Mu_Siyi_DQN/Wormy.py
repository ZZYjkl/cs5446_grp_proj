...
class GameState:
    ...
    alpha = INITIAL_ALPHA
    Steps = np.zeros(TOTAL_LIFES)
    Rewards = np.zeros((TOTAL_LIFES))
    int_reward = []
    ...
    def getIntReward(self):
        return self.int_reward
    ...
    def frame_step(self, input_actions):
        ...
        if newHead['x'] == self.apple['x'] and newHead['y'] == self.apple['y']:
            ...
            if (self.hang_out == 0):
                reward = 1
                if(self.getLifes()==TOTAL_LIFES):
                    self.int_reward.append(reward)
            self.size += 1
            self.time_out = 0
            if (self.size > 10):
        ...
        if newHead['x'] == -1 or newHead['x'] == CELLWIDTH or newHead['y'] == -1 or newHead['y'] == CELLHEIGHT:
            if self.getLifes()<TOTAL_LIFES:
                # print("Alive: ", self.alive_step)
                self.Steps[self.getLifes()] = int(self.alive_step)
                # print("Added: ", self.steps[self.getLifes()])
            reward = -1
            if (self.getLifes() == TOTAL_LIFES):
                self.int_reward.append(reward)
            self.n_reward += reward
            if self.getLifes() < TOTAL_LIFES:
                self.Rewards[self.getLifes()] = int(self.n_reward)
            terminal = True
            self.__init__()
...
def drawWorm(wormCoords):
    ...
    pygame.draw.rect(DISPLAYSURF, RED, wormHeadRect)
